...
Try<Owned<Backend>> Backend::create(const Flags& flags)
{
  ...
  if (!creators.contains(flags.appc_backend)) {
    return Error("Unknown or unsupported provisioner backend");
  }

  return creators[flags.appc_backend](flags);
}
...
Future<Nothing> CopyBackend::provision(
    const vector<StoredImage>& images,
    ...
    const string& directory)
{
  ...
  foreach (const StoredImage& image, images)
  {
    futures.push_back(
        futures.back().then(
...
Future<Nothing> CopyBackendProcess::_provision(
  ...
  const string& directory)
{
  LOG(INFO) << "Provisioning image layer '" << image.manifest.name() << "'"
            << " to " << directory;

  // Asynchronously copy the layer, overwriting any files that already
  // exist.
  // TODO(idownes): Control the IO rate for this.
  vector<string> argv{
    "cp",
    "--archive",
...
{
  // Asynchronously remove the directory because even removal of large
  // trees can take considerable time.
  // TODO(idownes): Control the IO rate for this.
