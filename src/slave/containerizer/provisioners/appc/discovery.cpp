...
Try<Owned<Discovery>> Discovery::create(const Flags& flags)
{
  ...
  if (!creators.contains(flags.appc_discovery)) {
    return Error("Unknown or unsupported strategy '" +
                 flags.appc_discovery + "'");
  }

  return creators[flags.appc_discovery](flags);
}
...
{
  Try<string> canonical = canonicalize(name, labels);
  ...
  return path::join(
      "file:///",
      flags.appc_discovery_local_dir,
      canonical.get() + ".aci");
}
