...
public:
  static Try<Owned<Chroot>> create(const string& rootfs)
  {
    ...
    // We don't want the bind mount to persist beyond the test so
    // we'll lazily umount it now. Because the test process has the
    // rootfs as its cwd the umount won't actually happen until the
    // TemporaryDirectoryTest::TearDown() changes back to the original
    // directory.
    Try<Nothing> unmount = fs::unmount(rootfs, MNT_DETACH);
    if (unmount.isError()) {
      return Error("Failed to lazily unmount chroot rootfs: " +
                   unmount.error());
    }
